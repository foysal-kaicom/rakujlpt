...
export default function PricingCard({ plan, subscribeModal }: any) {
  return (
    <div
      key={plan.id}
      className={`bg-white rounded-3xl p-8 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 relative w-[320px] ${
        plan.is_popular ? "ring-3 ring-purple-300" : "ring-3 ring-blue-50"
      }`}
    >
      {plan.is_popular && (
