...
<h3 class="text-xl font-semibold p-[12px] mb-[50px] rounded text-black bg-indigo-300">Create Agents</h3>

<form action="{{ route('agents.store') }}" method="POST" enctype="multipart/form-data" class="px-10">
  @csrf

  <div class="flex gap-10">

    <div class="w-full p-10 shadow-md border border-gray-100 rounded bg-white">

      <div class="flex items-start gap-10">
        <!-- Image Upload Section -->
        <div class="flex flex-col items-center text-center w-[220px]">
          <div class="relative border rounded bg-gray-100 flex items-center justify-center size-[200px] overflow-hidden">
            <img id="imagePreview" src="{{ asset('imagePH.png') }}" alt="Display Image" class="size-[200px] object-cover rounded border" />
          ...
          <label for="fileInput" class="mt-3 w-[200px] text-white py-2 px-3 rounded cursor-pointer bg-indigo-500 hover:bg-indigo-700 transition">
            Choose Image
          </label>
        ...
        <!-- Agent Details -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 w-[calc(100%-220px)]">
          ...
          <div>
            <label class="block text-sm font-medium mb-1">Commission Amount</label>
            <input
              type="number"
