...
@section('contents')
    <div class="bg-white rounded-3 shadow-sm border p-6">
        ...
        <!-- Form -->
        <form action="{{ route('agent.candidate-notices.store') }}" method="POST">
            @csrf
            ...
            <div class="mt-4">
                ...
                <textarea
                    ...
                    rows="5"
                    class="form-control form-control-lg shadow-sm rounded-2"
                    placeholder="Write notification message...">{{ old('message') }}</textarea>
                @error('message')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>

            <!-- Agent ID (Auto assign from auth) -->
