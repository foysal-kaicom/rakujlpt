<div id="sidebar"
    ...
    <div>
        ...
        @php
            ...
            $payment = request()->routeIs('agent.payment.*');
        ...
        <div
            ...
            <div class="menu-section" data-toggle="candidate-notices">
              ...
              <!-- Payment Section -->
        <div class="menu-section" data-toggle="payment">
            <div class="cursor-pointer">
                <div
                    class="group flex items-center justify-between px-3 py-2.5 rounded-xl transition-all duration-200 {{ $payment ? 'bg-emerald-500 text-white shadow-lg' : 'text-slate-700 hover:text-emerald-600 hover:bg-emerald-50' }}">
                    <div class="flex items-center gap-3">
                        <div
                            class="flex items-center justify-center w-9 h-9 rounded-lg {{ $payment ? 'bg-white/20' : 'bg-gradient-to-br from-emerald-500 to-emerald-600' }} text-white shadow-md group-hover:shadow-lg group-hover:scale-110 transition-all duration-200">
                            <i class="fa-solid fa-dollar-sign"></i>
                        </div>
                        <span class="font-semibold text-sm">Payment</span>
                    </div>
                    <i
                        class="fas fa-chevron-down drop-arrow text-xs {{ $payment ? 'text-white/80' : 'text-slate-400' }} transition-transform duration-300 {{ $payment ? 'rotated' : '-rotate-90' }}"></i>
                </div>
            </div>
            <div class="submenu mt-1 ml-12 space-y-1 {{ $payment ? '' : 'hidden' }}"
                data-target="payment">
