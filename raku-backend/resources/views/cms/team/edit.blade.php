...
<div class="flex gap-3 text-xs sm:text-sm font-semibold text-gray-500 items-center w-full mb-[20px]">
    <div class="flex gap-3 items-center">
        ...
        <p class="cursor-pointer line-clamp-1 text-blue-500">Edit Member</p>
    </div>
</div>

<h3 class="text-xl font-semibold p-[12px] mb-[50px] rounded text-black bg-indigo-300">
    Edit Team Member
</h3>

<form action="{{ route('our-team.update', $member->id) }}" method="POST" enctype="multipart/form-data" class="px-10">
    @csrf
    @method('PUT')
    ...
    <div class="flex gap-8">
        ...
        <div class="flex flex-col items-center text-center w-[260px] p-6 shadow-md border border-gray-100 rounded bg-white shrink-0">
            <div class="relative border rounded bg-gray-100 flex items-center justify-center size-[200px] overflow-hidden">
                ...
                <img id="imagePreview"
                     src="{{ $member->photo ? $member->photo : asset('imagePH.png') }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2">
                    ...
                    <input type="text" name="designation" value="{{ old('designation', $member->designation) }}"
                           class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                    @error('designation') <div class="text-red-500 text-sm">{{ $message }}</div> @enderror
                </div>
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="description" rows="3"
                              class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('description', $member->description) }}</textarea>
                ...
                <div>
                    ...
                    <input type="url" name="linkedin_url" value="{{ old('linkedin_url', $member->linkedin_url) }}"
                           class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                    @error('linkedin_url') <div class="text-red-500 text-sm">{{ $message }}</div> @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="url" name="facebook_url" value="{{ old('facebook_url', $member->facebook_url) }}"
                           class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                    @error('facebook_url') <div class="text-red-500 text-sm">{{ $message }}</div> @enderror
                </div>
