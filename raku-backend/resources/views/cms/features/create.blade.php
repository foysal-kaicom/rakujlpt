...
<div class="p-6 bg-white rounded-2xl shadow-md">
    ...
    <form action="{{ route('features.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="title" placeholder="e.g., Comprehensive Mock Tests"
        ...
        <div>
            ...
            <textarea name="description" rows="3"
                      class="w-full mt-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                      placeholder="Short description shown under the feature title">{{ old('description') }}</textarea>
            @error('description') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
        </div>

        {{-- Icon Upload --}}
        <div>
            <label class="block text-sm font-semibold text-gray-700">Icon (image)</label>
            <input type="file" name="icon"
                   class="w-full mt-1 block text-sm text-gray-600 file:mr-4 file:py-2 file:px-4
                          file:rounded-lg file:border-0 file:text-sm file:font-semibold
                          file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
            <p class="text-xs text-gray-500 mt-1">Upload SVG/PNG/JPG/WEBP (max 2MB).</p>
