...
<div class="bg-white rounded-3 shadow-sm border p-6">
    ...
    <div class="text-2xl font-bold text-gray-800 border-b pb-3 mb-5 flex items-center gap-2">
        <p class="p-2 rounded-full bg-indigo-500 text-white"> <svg xmlns="http://www.w3.org/2000/svg" class="size-6" viewBox="0 0 24 24" fill="currentColor">
    ...
    <!-- Form -->
    <form action="{{ route('faq.store') }}" method="POST" class="">
        @csrf
        ...
        <div class="row g-4">
            ...
            <div class="col-md-10">
                ...
                <input
                    ...
                    placeholder="e.g. Where should I contact?" />
                @error('question')
        ...
        <div class="mt-4">
            ...
            <textarea
                ...
                rows="5"
                class="form-control form-control-lg shadow-sm rounded-2"
                placeholder="Write a clear and helpful answer..."></textarea>
            @error('answer')
            <small class="text-danger">{{ $message }}</small>
            @enderror
        </div>
